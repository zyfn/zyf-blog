...
本文件是 `zyf-blog` 仓库唯一的项目级 Agent 契约，覆盖代码、UI、内容、验证、Git 与部署。仓库内不新增更深层的 `AGENTS.md`。

技术博客的写作、审稿和与 Agent 协作规范统一见 [docs/writing/技术博客写作指南.md](docs/writing/技术博客写作指南.md)。本文件只保留工程约束，不复制写作方法。
...
- 桌面标题与正文等宽，约占视口 60%，最大约 1040px；移动端使用可用宽度。
