...
- 只有需要 React 浏览器状态的组件才添加 `"use client"`，当前仅限搜索和打字机。主题切换和文章目录交互使用根布局的原生事件脚本，避免增加 Vinext 的 RSC Client Reference。
...
- 阅读目录行为只在 `ArticleToc` 中维护。桌面端默认固定在正文左侧阅读轨道，可收起但不自由拖拽或记忆任意位置；窄屏使用正文上方的紧凑折叠入口。目录必须保持键盘可用、内部可滚动，并正确暴露 `aria-expanded`。
...
- 文章阅读页采用接近 OpenAI 工程博客的中性编辑排版，不把首页和 About 的毛玻璃装饰带入正文。正文区域以黑、白、灰为主，强调色只用于链接、焦点和少量状态。
- 文章 H1 使用 `clamp(2.2rem, 3.2vw, 3rem)` 的响应式范围，桌面端不超过约 48px，字重以 520–560 为基准；中文标题需要比英文展示标题更克制，不使用超大字号、过粗字重或过紧字符间距。
- 正文以 16–17px、400 字重、约 1.7 行高为基准；桌面标题与正文等宽，约占视口 60%，最大约 1040px，平板和移动端使用可用宽度。H2 约 28–32px，H3 约 20–23px，标题字重控制在 520–580。代码继续使用等宽字体。
- 桌面文章布局使用左侧目录与正文阅读列；目录是安静的文字导航，不套毛玻璃卡片、不使用时间线圆点。正文标题、段落、表格和代码块保持平面化，章节间不依赖重复横线制造层级。
...
- H2/H3 标题直接表达主题，不使用 `一、`、`二、`、`1.`、`2.1` 等人工序号；目录层级由语义标题和 HTML heading 层级生成。
- 正文不使用 `---`、`***` 或 `<hr>` 作为章节分割线；层级通过标题、段落节奏和留白建立。
- 一篇长技术文章通常控制在 4–6 个 H2；只有认知路径确实需要时才增加，避免把每组参数或每个接口提升成独立主章节。
...
- 先用简短开篇说明对象是什么、解决什么问题、读者什么时候需要它，再进入协议和命令细节。
- 所有命令集中在一个 H2 章节，例如“命令完整指南”；命令分组和具体命令使用 H3/H4，不为每组命令创建新的 H2。
- 每个需要完整介绍的命令统一说明：用途、适用时机、关键入参、成功出参、相关通知或反向请求、最小可运行示例、失败与版本边界。
- 示例必须同时让读者看到请求和响应；涉及事件流时，再补充关键通知顺序。不要只给字段表，也不要只给无法运行的片段。
- 核心调用链给完整 Demo；低频或实验命令可以使用紧凑表格和最小请求/响应，明确实验状态与校验版本，避免为了“完整”重新写成无重点的 Schema 副本。
