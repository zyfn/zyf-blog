...
边界仍然要落到具体 Surface。Desktop、CLI 与 IDE 可以使用本机配置，Web / Cloud 不会自动继承本机 MCP；Hook 也只覆盖当前版本和 Surface 实际暴露的生命周期事件，不能假设所有托管工具与远程动作都会经过本地 Hook。Skill 脚本则继续受 Sandbox 与 Approval 控制。Hook 是确定性自动化和附加守卫，不是覆盖所有工具的安全边界。[MCP](https://learn.chatgpt.com/docs/extend/mcp) · [Hooks](https://learn.chatgpt.com/docs/hooks)
