...
tags:
  ...
  - Agent Infra
  - Developer Tools
cover: /images/posts/codex-agent-whitepaper/agent-loop.svg
featured: true
draft: false
---
...
正文按执行现场、控制面、完成证据、扩展拓扑和程序化接入依次展开。CLI、Desktop 与 IDE 的 Slash 命令放在文末附录。能力集合会随模型、账户、组织策略、客户端版本和实验开关变化；运行时应以 `/status`、Slash 菜单和当前官方文档为准。[Feature maturity](https://learn.chatgpt.com/docs/feature-maturity)
...
<Figure
  src="/images/posts/codex-agent-whitepaper/agent-loop.svg"
  alt="Codex 执行与控制循环，从目标和运行面经过上下文、行动与证据推进到完成协议，并由权限控制和状态模型支撑"
  caption="执行面决定行动在哪里发生，控制面限制行动并保存过程；二者共同决定结果能否被验证和恢复。"
/>
...
这张表描述的是任务形状，不是成熟度排名。成熟度必须落到具体能力上判断：App Server 的默认协议面与实验 API 有明确边界，WebSocket transport 仍被官方标为 experimental；不能把某个子命令的标签外推为整个 Surface 的成熟度。
...
Cloud environment 必须显式准备依赖、环境变量、网络和启动脚本。Setup 阶段与 Agent 阶段可能使用不同网络策略，Secret 也不能被假定为始终可读。

把任务发送到 Cloud，并不会复制本机的隐含前提。迁移前应先把本机服务、私有依赖和凭证来源写成可重建条件。[Local environment](https://learn.chatgpt.com/docs/environments/local-environment) · [Cloud environment](https://learn.chatgpt.com/docs/environments/cloud-environment)

#### Desktop 适合监督，CLI 适合贴近 Shell
...
配置会按 CLI 覆盖、可信项目配置、Profile、用户配置、系统配置和内置默认值逐层合并。不可信项目不会加载项目级 Config、Hook 与 Rule。

组织的 `requirements.toml` 也不是另一份默认配置，而是用户可选范围的上限。排查“配置为什么没生效”时，要查看最终来源链，不能只检查一个文件。[Config basics](https://learn.chatgpt.com/docs/config-file/config-basic)
...
### Sandbox、Approval 与 Auto-review 各管一层
...
把 `danger-full-access` 与 `approval_policy = "never"` 同时启用，会同时拿掉系统隔离和交互闸门。这种组合只适合调用者已经提供强隔离、没有敏感凭证且可以整体丢弃的环境；它不应成为日常开发的“省事模式”。[Approvals and security](https://learn.chatgpt.com/docs/agent-approvals-security) · [Sandboxing](https://learn.chatgpt.com/docs/sandboxing)

Permission profiles 用细粒度文件系统和网络规则组合执行边界，但它与传统 `sandbox_mode` 不是简单的两层叠加。迁移前应确认当前实际生效的是哪套策略；两个配置同时存在，并不能证明获得了“双重保护”。[Permissions](https://learn.chatgpt.com/docs/permissions)
...
扩展边界必须落到具体 Surface。Desktop、CLI 与 IDE 可以使用本机配置，Web / Cloud 不会自动继承本机 MCP；Hook 也只覆盖当前版本和 Surface 实际暴露的生命周期事件。

因此，不能假设所有托管工具和远程动作都会经过本地 Hook。Hook 适合确定性自动化和附加守卫，不是覆盖所有工具的安全边界；Skill 脚本仍要经过 Sandbox 与 Approval。[MCP](https://learn.chatgpt.com/docs/extend/mcp) · [Hooks](https://learn.chatgpt.com/docs/hooks)
...
多 Agent 的可用入口和配置格式仍会演进。Subagent 分别消耗上下文与模型调用；在非交互执行中，新的审批请求无法临时等待用户处理。因此，委派前要准备所需权限，不能指望后台 Worker 在关键步骤停下来等人点击确认。

#### Worktree 解决文件所有权，不解决所有共享状态
...
#### Goal 与后台终端解决持续性，不等于 Durable Workflow
...
| Python SDK | 本地 App Server 的高层封装 | Python 中控制 Thread、Turn 和事件 | 携带固定 CLI Runtime；版本边界以当前 SDK 文档为准 |
...
### SDK：先分清它封装的是 CLI 还是 App Server

TypeScript SDK 通过标准输入输出与本地 CLI 交换 JSONL；Python SDK 控制本地 App Server。两者都便于在代码中启动、恢复和消费 Agent 任务，但都不是免运维的远程 Codex API。

选择 SDK 前，先确认调用者需要的是一次执行、Thread 生命周期，还是完整客户端协议。[Codex SDK](https://learn.chatgpt.com/docs/codex-sdk) · [TypeScript SDK source](https://github.com/openai/codex/tree/main/sdk/typescript) · [Python SDK source](https://github.com/openai/codex/tree/main/sdk/python)

平台仍需负责工作目录、资源限制、身份、凭证、持久状态、超时、重试、审计和人工接管。SDK 提供 Agent 能力，不自动提供生产可靠性。

### App Server：当你真的要构建客户端
...
Managed configuration 和 requirements 用来限制个人配置能够偏离的范围；Analytics、Compliance 和 Audit 则提供采用、用量和事件证据。它们能说明系统怎样被使用，不能直接证明代码质量或工程生产力。[Governance](https://learn.chatgpt.com/docs/enterprise/governance) · [Managed configuration](https://learn.chatgpt.com/docs/enterprise/managed-configuration)
...
Codex 能把代码理解、文件修改、命令执行与验证连接成持续循环，但可靠性不由模型单独决定。开始前要明确环境、状态和权限的所有者；执行中要让 Agent 获得窄而真实的反馈；结束时要用外部证据接收结果。只有这些责任已经能被解释、约束和接管，并发与自动化才会扩大产出，而不是扩大模糊处。
